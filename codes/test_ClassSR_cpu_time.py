...
for test_loader in test_loaders:
    ...
    idx = 0
    num_ress = [0, 0, 0]
    processing_time, to_tensor_time, inference_time, postprecessing_time = [], [], [], []
    warm_up = False
    ...
    for idx, data in enumerate(test_loader):
        ...
        # warm up for cpu inference
        if warm_up and idx == 0:
            for _ in range(5):
                print('warm up for inference')
                model.test()

        img_path = data['GT_path'][0] if need_GT else data['LQ_path'][0]
        img_name = osp.splitext(osp.basename(img_path))[0]

        pre_time, tensor_time, i_time, post_time = model.test()
        processing_time.append(pre_time)
        to_tensor_time.append(tensor_time)
        inference_time.append(i_time)
        postprecessing_time.append(post_time)
        logger.info('# processing_time, to_tensor_time, inference_time, postprecessing_time # {0} {1} {2} {3}'.format(pre_time, tensor_time, i_time, post_time))
    ...
    if len(processing_time) > 0:
        # processing_time, to_tensor_time, inference_time, postprecessing_time
        processing_time = np.array(processing_time).mean()
        to_tensor_time = np.array(to_tensor_time).mean()
        inference_time = np.array(inference_time).mean()
        postprecessing_time = np.array(postprecessing_time).mean()
        total_time = processing_time + to_tensor_time + inference_time + postprecessing_time
        
        logger.info('# processing_time, to_tensor_time, inference_time, postprecessing_time # {0} {1} {2} {3} all: {4}'.format(processing_time, to_tensor_time, inference_time, postprecessing_time, total_time))
